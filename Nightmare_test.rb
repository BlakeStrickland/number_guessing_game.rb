...
array = [number]
median = 0
average_string_length = input.length
...
#begins loop to check if user input is "" or nil
    loop do
      if input.empty? || input.nil?
        ...
        string_length1 = ("#{average_string_length}" + "#{string_length}")
        string_length2 = ("#{string_length1}"/"#{count}")
        puts "The average string length is: #{string_length2}"
        ...
        variance_step1 = (number-average)**2
        variance_step2 = (variance_step1/count)
        standard_dev = Math.sqrt(variance_step2)
        puts "The standard deviation is: #{standard_dev}"

        #median_part1 = array.sort
        #median_part2 = array[(count/2)]

        #puts "The median is: #{median_part2}"
        median = array.sort
          if median.to_s % 2 != 0
            (median.length + 1) / 2.0
            puts "This is the median: #{median}"
          else
            ((median.length/2.0) + ((median.length + 2)/2.0) / 2.0)
            puts "is the median: #{median}"
          end
